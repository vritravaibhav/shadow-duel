...
class Fighter extends PositionComponent with HasGameReference<ShadowGame> {
  ...
  double get damageTakenMult => weapon.special == Special.radiance ? 0.6 : 1.0;
  ...
  void startMove(MoveKind k) {
    if (!alive || state == FState.dead || state == FState.victory || state == FState.hit) {
      return;
    }
  ...
  void update(double dt) {
    ...
    final adt = slowed && state != FState.dead ? dt * 0.6 : dt;
    stateT += adt;
  ...
  void _receiveHit(Fighter from, MoveSpec m, bool blocked) {
    ...
    var dmg = m.dmg * from.dmgScale * damageTakenMult;
